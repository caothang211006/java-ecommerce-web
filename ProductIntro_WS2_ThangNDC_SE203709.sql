...
drop database ProductIntro_WS2_ThangNDC_SE203709
go
...
CREATE TABLE cart (
    account varchar(20) references accounts(account) ON DELETE CASCADE,
...
CREATE TABLE orders (
    ...
    account varchar(20) references accounts(account),
    orderDate datetime default CURRENT_TIMESTAMP,
    address nvarchar(500) not null,
    phone nvarchar(20) not null,
    status int default 0  -- 0: chờ xử lý, 1: đang giao, 2: hoàn thành, 3: hủy
)
